...
{   
    ...
    //PID - TODO tune
    // probably have to create separate PID for rollers??
    ...
    private static final InvertedValue MOTOR_INVERTED = InvertedValue.CounterClockwise_Positive;// will have to change this because it matters what side the motor is on
  ...
  public LeftIntake()
  {
    intake_pivot_motor = new TalonFX(Constants.CAN_ID.INTAKE_LEFT_ROTATE);//need to set CAN ID might have to change how it is set because of different motors
        configureIntakeMotor(); //TODO Need pass throughs
  ...
  private void configureIntakeMotor() 
    {
        ...
        intakeConfigs.Slot0.kS = SLOT0_KS;
        intakeConfigs.Slot0.kV = SLOT0_KV;
        intakeConfigs.Slot0.kP = SLOT0_KP;
        intakeConfigs.Slot0.kI = SLOT0_KI;
        intakeConfigs.Slot0.kD = SLOT0_KD;
        intakeConfigs.MotorOutput.Inverted = InvertedValue.Clockwise_Positive;// might need to tweek this
        intakeConfigs.Voltage
        ...
        for (int i = 0; i < CONFIG_RETRIES; ++i) {
            status = intake_pivot_motor.getConfigurator().apply(intakeConfigs);
            if (status.isOK()) {
                break;
            }
        }
        if (!status.isOK()) {
            System.out.println("Could not apply intake configs, error code: " + status);
        }
    }
}
